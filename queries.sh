...
# Bash style arithmetic | Inefficent and Readability issues
...
echo "$($PSQL "SELECT name FROM games AS g JOIN teams AS t ON t.team_id = g.winner_id OR t.team_id = g.opponent_id WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT DISTINCT name FROM games AS g JOIN teams AS t ON g.winner_id = t.team_id ORDER BY name")"

echo -e "\nYear and team name of all the champions:"
echo "$($PSQL "SELECT year, name FROM games AS g JOIN teams AS t ON t.team_id = g.winner_id WHERE round = 'Final' ORDER BY year")"
